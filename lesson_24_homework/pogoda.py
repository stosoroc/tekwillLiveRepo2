...
def get_geo_location_from_ip(ip):
    response = requests.get(f"http://api.ipstack.com/{ip}?access_key="+get_api_key("ipstack"))
    data = response.json()
...
def temperatura(coordinates):
    url = 'http://api.weatherapi.com/v1/current.json?'+'&key='+get_api_key("weatherapi")+'&q='+coordinates
    ...
    print(f"In {country}, {name} avem {temp_c}({feelslike_c})grade, cu umiditatea de {humidity}%")
